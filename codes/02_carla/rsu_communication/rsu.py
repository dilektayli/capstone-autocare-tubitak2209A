...
class RSUCommunicationModule:
    ...
    def send_data(self, data, destination_vehicle_id=None):
        if self.socket:
            for vehicle in RSUCommunicationModule.connected_rsus:
                if destination_vehicle_id is None or vehicle.rsu_id == destination_vehicle_id:
                    message = f"RSU{self.rsu_id}:{data}"
                    vehicle.socket.sendto(message.encode(), (vehicle.host, vehicle.port))
                    print(f"Data sent from RSU {self.rsu_id} to Vehicle
                     {vehicle.rsu_id}: {data}")
        else:
