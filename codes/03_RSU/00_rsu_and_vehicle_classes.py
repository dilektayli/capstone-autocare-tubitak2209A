...
class Vehicle:
    ...
    def connect(self):
        ...
        print(f"Vehicle {self.vehicle_id} connected on {self.host}:{self.port}")

    def receive_data(self):
        if self.socket:
            data, _ = self.socket.recvfrom(1024)
            sender_id, message = data.decode().split(":", 1)
            print(f"Received data from {sender_id}: {message}")
        else:
...
if __name__ == "__main__":
    ...
    # Connect vehicles with the correct port
    vehicle1.port = 12345
    vehicle2.port = 12346
    vehicle3.port = 12347
    vehicle4.port = 12348
    ...
    rsu1.connected_vehicles = [vehicle2, vehicle3, vehicle4]
    rsu2.connected_vehicles = [vehicle1, vehicle2]

    # Example data sending from RSUs to connected vehicles
    rsu1.send_data("Selam", destination_vehicle_ids=[vehicle2.vehicle_id, vehicle3.vehicle_id, vehicle4.vehicle_id], sender_rsu_id=1)
    rsu2.send_data("Sanane", destination_vehicle_ids=[vehicle1.vehicle_id, vehicle2.vehicle_id], sender_rsu_id=2)
